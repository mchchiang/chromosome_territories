...
run=$3            # trial number
num_beads=$4      # number of beads
run_dir=$5        # run directory

# Init config (default is random walk)
gen_chromo_exe="../bin/exe/GenRandomWalk"
...
sim_name="wall_L_${box_size}_HH_${e_hethet}_HL_${e_hetlam}_run_${run}"
...
if [ $wall == "bead" ]; then
    ...
    cp wall-bead.lam $file
else
    echo "Copying lj wall lammps script"
    cp wall-ljwall.lam $file
fi
...
${gen_chromo_exe} $num_beads $init_box_size $init_box_size $init_box_size $buffer "${run_dir}/${init_file}" "${run_dir}/${map_file}"
